...
public class LeagueTableBuilderTests
{
    ...
    public void BuildFullLeagueTable_should_set_position_and_status()
    {
        ...
        var leagueTable = builder.BuildFullLeagueTable(competition.Id);
        ...
        leagueTable.Table.Length.Should().Be(4);
        leagueTable.Table.Select(x => x.Status).Distinct().Single().Should().Be("assigned-status");
        leagueTable.Table.Select(x => x.Position).Should().BeEquivalentTo(new List<int> { 1, 2, 3, 4});
    }
    ...
    public void BuildPartialLeagueTable_should_set_position_but_not_status()
    {
        ...
        leagueTable.Table.Length.Should().Be(4);
        leagueTable.Table.Select(x => x.Status).Distinct().Single().Should().BeNull();
        leagueTable.Table.Select(x => x.Position).Should().BeEquivalentTo(new List<int> { 1, 2, 3, 4});
    }
    ...
    private static List<LeagueTableRow> GetExpectedRows(string? status = null)
    {
